...
struct GameFeature: ReducerProtocol {
    ...
    struct State: Equatable {
        ...
        var recordState: RecordFeature.State? = nil
    }
    ...
    var body: some ReducerProtocol<State, Action> {
        Reduce { state, action in
            ...
            case .setNavigation(let isActive):
                ...
                state.recordState = isActive ? RecordFeature.State() : nil
                return.none
        ...
        .ifLet(\.recordState, action: /Action.record) {
            RecordFeature(recordService: recordService)
        }
    }
}
